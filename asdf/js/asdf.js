...
(function() {
    ...
    Asdf.prototype.mode = {
       ...
       getMode : function() {
        ...
        switchMode : function(text) {
			if (text == 'hell')
				_this.dom.modetext.innerHTML = '최상';   
			else if (text == 'normal')
				_this.dom.modetext.innerHTML = '일반';   
			else
				_this.dom.modetext.innerHTML = '중간';   
        ...
        show : function(o) {
           ...
           if (o.type == 'lose') {
               ...
               _this.dom.message.innerHTML = 
                   ...
                   '<h2>게임 종료</h2>'+
                   '<p>' + 
                     '<b>점수</b> <span class="info-style">' + _this.dom.score.innerHTML + '</span> '+
                     '<b>레벨</b> <span class="info-style">' + level + '</span>'+
                   
                     '<b>모드</b> <span class="info-style">' + _this.mode.getMode() + '</span>'+
                   '</p>'+
                   '<hr>'+
                   '<hr class="small">'+
                   '<p>'+
                     '<button class="key '+type+'">'+type+'</button>가 다음키 였습니다 ,'+
                   '</p>'+
                   '<p>'+
                   '그렇지만 <button class="key '+o.key+'"">'+o.key+'</button>키를 누르셨습니다 <b class="smiley">D:</b>'+
                   '</p>'+
                   '</div>';
           }
           ...
           if (o.type == 'win') {
               ...
               _this.dom.message.innerHTML = '<div class="container"><h1>레벨 완료</h1></div>';
           }
    ...
    Asdf.prototype.game = {
        ...
        next : function() {
            ...
            // Show message
            _this.message.show({type : 'levelmode', text : '레벨 ' + _this.mode.getLevel() + ' <span class="info-style-2">' + _this.mode.getMode() + '</span>'});
